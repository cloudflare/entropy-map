...
pub struct MapTuple<K, V, const B: usize = 32, const S: usize = 8, ST = u8, H = WyHash>
where
    ST: PrimInt + Unsigned,
    H: Hasher + Default,
{
    ...
    H: Hasher + Default,
{
    ...
    /// # use entropy_map::Map;
    /// let map = MapTuple::try_from(HashMap::from([(1, 2), (3, 4)])).unwrap();
    ...
    /// # use std::collections::HashMap;
    /// # use entropy_map::Map;
    /// let map = MapTuple::try_from(HashMap::from([(0, 0); 0])).unwrap();
...
impl<K, V, const B: usize, const S: usize, ST, H> ArchivedMapTuple<K, V, B, S, ST, H>
where
    ...
    H: Hasher + Default,
{
    ...
    /// ```
    /// # use std::collections::HashMap;
    /// # use entropy_map::ArchivedMapTuple;
    /// # use entropy_map::Map;
    /// let map = MapTuple::try_from(HashMap::from([(1, 2), (3, 4)])).unwrap();
    /// let bytes = rkyv::to_bytes::<rkyv::rancor::Error>(&map).unwrap();
    /// let archived_map = rkyv::access::<ArchivedMapTuple<u32, u32>, rkyv::rancor::Error>(&bytes).unwrap();
...
mod tests {
    ...
    fn test_map_with_dict() {
        ...
        let map = MapTuple::try_from(original_map.clone()).unwrap();
    ...
    fn test_rkyv() {
        ...
        let rkyv_map = rkyv::access::<ArchivedMapTuple<u64, u32>, rkyv::rancor::Error>(&rkyv_bytes).unwrap();
                ...
                paste! {
                    proptest! {
                        ...
                        fn [<proptest_map_with_dict_model_ $b _ $s _ $gamma>](model: HashMap<u64, u64>, arbitrary: HashSet<u64>) {
                            let entropy_map: MapTuple<u64, u64, $b, $s> = MapTuple::from_iter_with_params(
                                model.clone(),
                                $gamma as f32 / 100.0
                            ).unwrap();
